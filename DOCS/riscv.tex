...
The non-pipelined processor executes at top 0.5 MIPS/MHz because each instruction fetch and execution requires a clock cycle. The pipelined processors execute at top 1 MIPS/MHz (jump and branches taken and reading ROM/RAM for data requires an extra clock cycle).
...
\begin{itemize}
\item CYCLE and CYCLEH -- these 32-bit register form a 64-bit value that contains the counted clock cycles since the last reset.
...
\end{lstlisting}
This will set the architecture to RV32I and the ABI to ilp32. This means that integers, long integers and pointers use 32-bit entities. The destination directory is \lstinline|/opt/riscv32|
