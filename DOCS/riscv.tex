...
In the processor, a simple three-state FSM is used. All instructions require two clock cycles to be fetched and executed, and an extra clock cycle is needed when reading RAM or ROM. In the pipelined processor, the PC points to +4 of the currently executing instruction. The FSM takes care of that.
...
\item  \texttt{qsort} -- sorts an integer array using the \texttt{qsort} C library function (seems to work).
