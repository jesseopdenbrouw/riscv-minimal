...
It is possible to compile the toolchain to only use register \texttt{x0} to \texttt{x15}. This is called the RISC-V E extension. As a positive side effect, the register file can be cut down from 32 registers to 16 registers. This will lower the gate count and possible speed up the device. A negative side effect is that the pressure on register allocation is higher, possibly increasing instruction count when saving registers on the stack.
...
\begin{lstlisting}[language=]
riscv32-unknown-elf-gcc -O0 -g -o string string.c -Wall -T ../ldfiles/riscv.ld -march=rv32e -mabi=ilp32e -nostartfiles --specs=nano.specs ../crt/startup.c ../crt/startup.S
\end{lstlisting}
...
\item Implement ROM in onboard RAM with the use of a MIF file. This will lower the cell count, increase the overall speed, but each ROM read requires two clock cycles.
